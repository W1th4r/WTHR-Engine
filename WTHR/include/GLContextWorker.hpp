...
public:
  explicit GLContextWorker(GLFWwindow *sharedWindow) {
    if (!sharedWindow) {
      throw std::invalid_argument(
          "GLContextWorker: sharedWindow cannot be null");
    }

    std::cout << "[GLContextWorker] Creating shared context..." << std::endl;

    // Store original hints
    int originalVisible = 1, originalVersionMajor = 4, originalVersionMinor = 6,
        originalProfile = GLFW_OPENGL_CORE_PROFILE;
    ...
    try {
      ...
      workerWindow =
          glfwCreateWindow(1, 1, "GPU Worker", nullptr, sharedWindow);
      if (!workerWindow) {
        const char *errorDesc;
        int errorCode = glfwGetError(&errorDesc);
        std::stringstream ss;
        ss << "Failed to create GPU worker context (Error " << errorCode << ": "
           << errorDesc << ")";
        throw std::runtime_error(ss.str());
      }

      std::cout << "[GLContextWorker] Worker context created successfully"
                << std::endl;

      // Verify context sharing
      if (!VerifyContextSharing(sharedWindow)) {
        throw std::runtime_error("Context sharing verification failed - OpenGL "
                                 "objects will not be shared between contexts");
      }
  ...
  void AddJob(std::function<void()> job) {
    if (!job) {
      std::cerr << "[GLContextWorker] Warning: Attempted to add null job"
                << std::endl;
      return;
    }

    {
      ...
      if (shouldExit) {
        std::cerr
            << "[GLContextWorker] Warning: Adding job after shutdown requested"
            << std::endl;
        return;
      }
  ...
  // Async model loading
  void AsyncLoadModel(std::shared_ptr<ThreadSafeModel> modelWrapper,
                      const std::string &path) {
    if (!modelWrapper) {
      std::cerr
          << "[GLContextWorker] Error: modelWrapper is null in AsyncLoadModel"
          << std::endl;
      return;
    }
    ...
    if (path.empty()) {
      std::cerr
          << "[GLContextWorker] Error: Empty path provided for model loading"
          << std::endl;
      return;
    }

    std::cout << "[GLContextWorker] Starting async model load: " << path
              << std::endl;
    ...
    std::thread loaderThread([this, modelWrapper, path]() {
      try {
        ...
        std::cout
            << "[GLContextWorker] CPU work completed, queuing GPU upload..."
            << std::endl;
        ...
        this->AddJob([modelWrapper, path]() {
          ...
          if (error != GL_NO_ERROR) {
            std::cerr
                << "[GLContextWorker] OpenGL error before model creation: "
                << error << std::endl;
          }
          ...
          try {
            ...
            if (error != GL_NO_ERROR) {
              std::cerr
                  << "[GLContextWorker] OpenGL error during model creation: "
                  << error << std::endl;
            }

            modelWrapper->SetModel(std::move(newModel));
            std::cout << "[GLContextWorker] Async mesh uploaded successfully!"
                      << std::endl;
            auto end = std::chrono::steady_clock::now();

            modelWrapper->m_LoadTime =
                std::chrono::duration<double>(end - start);
            if (std::filesystem::exists(path)) {
              modelWrapper->m_FileSize =
                  std::filesystem::file_size(path) / 1024 / 1024;
            }
            modelWrapper->GenerateThumbnail();
          } catch (const std::exception &e) {
            std::cerr
                << "[GLContextWorker] Failed to load model asynchronously: "
                << e.what() << std::endl;
            modelWrapper->Reset();
          ...
          error = glGetError();
          if (error != GL_NO_ERROR) {
            std::cerr << "[GLContextWorker] OpenGL error after async load: "
                      << error << std::endl;
          }
        });

      } catch (const std::exception &e) {
        std::cerr << "[GLContextWorker] Exception in loader thread: "
                  << e.what() << std::endl;
      }
    });
...
private:
  ...
  bool VerifyContextSharing(GLFWwindow *mainWindow) {
    ...
    const char *mainVersion =
        reinterpret_cast<const char *>(glGetString(GL_VERSION));
    const char *mainVendor =
        reinterpret_cast<const char *>(glGetString(GL_VENDOR));

    glfwMakeContextCurrent(workerWindow);
    const char *workerVersion =
        reinterpret_cast<const char *>(glGetString(GL_VERSION));
    const char *workerVendor =
        reinterpret_cast<const char *>(glGetString(GL_VENDOR));

    glfwMakeContextCurrent(mainWindow); // Restore main context

    bool versionsMatch =
        (mainVersion && workerVersion &&
         std::string(mainVersion) == std::string(workerVersion));
    bool vendorsMatch = (mainVendor && workerVendor &&
                         std::string(mainVendor) == std::string(workerVendor));

    std::cout << "[GLContextWorker] Main Context: "
              << (mainVendor ? mainVendor : "Unknown") << " - "
              << (mainVersion ? mainVersion : "Unknown") << std::endl;
    std::cout << "[GLContextWorker] Worker Context: "
              << (workerVendor ? workerVendor : "Unknown") << " - "
              << (workerVersion ? workerVersion : "Unknown") << std::endl;

    if (!versionsMatch || !vendorsMatch) {
      std::cerr << "[GLContextWorker] Warning: Context version/vendor mismatch "
                   "- sharing may not work properly"
                << std::endl;
      return false;
    }

    std::cout << "[GLContextWorker] Context sharing verified successfully"
              << std::endl;
    return true;
  }
  ...
  void WaitForInitialization() {
    ...
    if (!initCV.wait_for(lock, std::chrono::seconds(5),
                         [this] { return workerInitialized; })) {
      throw std::runtime_error(
          "GPU worker thread failed to initialize within timeout");
    }
  }
  ...
  void ThreadLoop() {
    try {
      ...
      if (error != GL_NO_ERROR || testVAO == 0) {
        throw std::runtime_error(
            "OpenGL functionality test failed in worker thread");
      }
...
#ifdef _WIN32
      std::cout << "[GLContextWorker] Worker thread initialized successfully - "
                   "OpenGL "
      ...
      while (!shouldExit && !glfwWindowShouldClose(workerWindow)) {
        ...
        // Wait for job or shutdown
        {
          ...
          cv.wait(lock, [this] { return !renderQueue.empty() || shouldExit; });

          if (shouldExit && renderQueue.empty())
            break;
        ...
        if (job) {
          try {
            ...
            std::cerr << "[GLContextWorker] Exception in job execution: "
                      << e.what() << std::endl;
          }
        }
      ...
      std::cerr << "[GLContextWorker] Fatal error in worker thread: "
                << e.what() << std::endl;
